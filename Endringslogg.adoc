...
* *UML-diagram*: UML-diagrammene i denne versjonen brukes bare til <<Del2>> og ikke <<Del1>>. De er dessuten deskriptive og ikke normative i denne versjonen (en praksis som også EU-standardene i dette området nå bruker), mens de var normative i forrige versjon.
...
*** «publisert av» er lagt til, for å kunne oppgi virksomheten som har publisert begrepsbeskrivelsen.
...
Endringene fra https://data.norge.no/specification/skos-ap-no-begrep/[_SKOS-AP-NO-Begrep – Forvaltningsstandard for tilgjengeliggjøring av begrepsbeskrivelser basert på SKOS_] (v.1.1.1) til <<Del2>> i denne versjonen:
